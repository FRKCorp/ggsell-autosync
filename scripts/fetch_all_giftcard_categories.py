"""Выгружает ПОЛНЫЙ список категорий подарочных карт FazerCards (все
страницы) в JSON — для сверки со списком клиента и получения точных
category_id.

Запуск: python scripts/fetch_all_giftcard_categories.py
Результат: data/fz_giftcard_categories.json
"""
...
def main() -> None:
    ...
    with FazerCardsClient(api_key=api_key, base_url=base_url) as client:
        while True:
            page += 1
            data = client.list_giftcard_categories(limit=50, cursor=cursor)
    ...
    out_path = os.path.join(out_dir, "fz_giftcard_categories.json")
